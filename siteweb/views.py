...
from . models import Jogador, Noticia, Sobre, Jogo, Informacao
...
def index(request):
     context = {
        "noticias_vasco": Noticia.objects.first(),
...
def elenco(request):
    context = {
        "elenco_vasco": Jogador.objects.all(),
    }
    return render(request, "website/elenco.html", context)

def jogador(request, id_jogador):
    jogador = get_object_or_404(Jogador, id=id_jogador)
    context = {
        "jogador": jogador,
    }
    return render(request, "website/jogador.html", context)
...
def noticia(request, id_noticia):
    ...
    return render(request, "website/noticias.html", context)

def sobre(request):
    context = {
        "sobre_site": Sobre.objects.all(),
    }
